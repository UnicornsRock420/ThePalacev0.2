...
public static class IptscraeEngine
{
    ...
    internal static readonly IReadOnlyDictionary<string, object> _iptCommands = new Dictionary<string, object>
    {
        ...
        { "BREAK", (IptCommandFnc)((iptTracking, recursionDepth) => { iptTracking.Flags |= IptTrackingFlags.Break; }) },
        {
            "RETURN",
        ...
        },
        {
            "GLOBAL", (IptCommandFnc)((iptTracking, recursionDepth) =>
            {
                ...
                {
                    var flags = iptTracking.Variables[key].Flags;
                    ...
                    iptTracking.Variables[key].Flags |= IptMetaVariableFlags.IsGlobal;
                }
            })
        },
    ...
    internal static IptVariable getVariable(IptTracking iptTracking, IptVariable variable)
    {
        ...
        if (iptTracking.Variables.ContainsKey(key) &&
            iptTracking.Variables[key].Variable.Type != IptVariableTypes.Shadow)
            return iptTracking.Variables[key].Variable;
        ...
        int recursionDepth = 0, IptMetaVariableFlags flags = IptMetaVariableFlags.None)
    {
        ...
        if (iptTracking.Variables.ContainsKey(key))
        {
            ...
            if (iptTracking.Variables[key].IsSpecial &&
                iptTracking.Variables[key].Variable.Type != variable.Type)
                throw new Exception($"Wrong datatype {iptTracking.Variables[key].Variable.Type}, {variable.Type}...");

            iptTracking.Variables[key].Variable = variable;
        }
        else
        {
    ...
    public static void Executor(IptAtomList AtomList, IptTracking iptTracking, int recursionDepth = 0)
    {
        ...
        foreach (var key in iptTracking.Variables.Keys)
        {
            ...
            if (((flags & IptMetaVariableFlags.IsGlobal) == 0) &&
                ...
                (recursionDepth == 0 || iptTracking.Variables[key].Depth >= recursionDepth))
