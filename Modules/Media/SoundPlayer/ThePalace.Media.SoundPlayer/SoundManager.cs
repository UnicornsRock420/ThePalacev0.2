...
public class SoundManager : Singleton<SoundManager>, IDisposable
{
    ...
    static SoundManager()
    {
        _libVlc = new LibVLC("--file-caching=0"); //--input-repeat=#
    }

    public SoundManager()
    {
        _libVlcPlayer = new MediaPlayer(_libVlc);
        _libVlcMedia = new ConcurrentDictionary<string, SndLog>();
    }
    ...
    public void Dispose()
    {
        ...
        _libVlcMedia.ToList().ForEach(m =>
        {
            try
            {
                m.Value.Media.Dispose();
            }
            catch
            {
            }
    ...
    {
        if (File.Exists(path)) _playSound(path, 0, (int)flags);
    }
    ...
    private class SndLog
    {
        ...
        public LibVLCSharp.Shared.Media Media { get; }
    }
}