...
public class BO_SERVERDOWN : IEventHandler<MSG_SERVERDOWN>
{
    public async Task<object?> Handle(object? sender, IEventParams @event)
    {
        if (sender is not IUserSessionState sessionState ||
            @event is not ProtocolEventParams { Request: MSG_SERVERDOWN inboundPacket } @params) return null;

        LoggerHub.Current.Debug(nameof(BO_SERVERDOWN) + $"[{@params.SourceID}]: {@params.RefNum}");
