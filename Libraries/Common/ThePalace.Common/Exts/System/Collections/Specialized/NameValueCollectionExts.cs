...
public static class NameValueCollectionExts
{
    ...
    {
        return collection?.AllKeys?.ToDictionary(k => k, k => collection?.GetValues(k)?.Take(1)?.FirstOrDefault());
    }

    public static Dictionary<string, string> ToDictionaryLastValue(this NameValueCollection collection)
    {
        return collection?.AllKeys?.ToDictionary(k => k, k => collection?.GetValues(k)?.LastOrDefault());
    }
    ...
    public static List<KeyValuePair<string, string>> ToListFirstValue(this NameValueCollection collection)
    {
        return collection?.AllKeys
            ?.Select(k => KeyValuePair.Create(k, collection?.GetValues(k)?.Take(1)?.FirstOrDefault()))?.ToList();
    }

    public static List<KeyValuePair<string, string>> ToListLastValue(this NameValueCollection collection)
    {
        return collection?.AllKeys?.Select(k => KeyValuePair.Create(k, collection?.GetValues(k)?.LastOrDefault()))
            ?.ToList();
    }

    public static List<KeyValuePair<string, string>> ToListAllValues(this NameValueCollection collection)
    {
        return collection?.AllKeys?.SelectMany(k => collection?.GetValues(k)?.Select(v => KeyValuePair.Create(k, v)))
            ?.ToList();
    }
    ...
    {
        collection2?.AllKeys?.ToList()
            ?.ForEach(k => collection1?.Add(k, collection2?.GetValues(k)?.Take(1)?.FirstOrDefault()));
    }
    ...
    {
        collection2?.AllKeys?.SelectMany(k => collection2?.GetValues(k)?.Select(v => KeyValuePair.Create(k, v)))
            ?.ToList()?.ForEach(k => collection1?.Add(k.Key, k.Value));
    }
}