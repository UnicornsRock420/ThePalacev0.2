...
namespace ThePalace.Core.Types
{
    ...
    public partial struct PString : IString, IComparable, IFormattable, IConvertible
    {
        [IgnoreDataMember]
        public const uint8 LengthSize = 1;
        [IgnoreDataMember]
        public const uint8 MaxLength = 0xFF - LengthSize;

        [IgnoreDataMember]
        private List<uint8>? _value = null;

        public static explicit operator uint8[](PString p) => p.Value ?? [];
        public static explicit operator char[](PString p) => p.ToString()?.ToCharArray() ?? [];
        public static explicit operator string(PString p) => p.ToString() ?? string.Empty;
        public static explicit operator PString(uint8[] v) => new(v);
        public static explicit operator PString(char[] v) => new(v);
        public static explicit operator PString(string v) => new(v);

        public PString(uint8[] value)
        {
        ...
        public PString(string? value = null)
        {
            var length = value?.Length ?? 0;
            if (length >= MaxLength)
            {
