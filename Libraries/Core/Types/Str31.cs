...
namespace ThePalace.Core.Types
{
    ...
    public partial struct Str31 : IString, IComparable, IFormattable, IConvertible
    {
        [IgnoreDataMember]
        public const uint8 LengthSize = 1;
        [IgnoreDataMember]
        public const uint8 MaxLength = 0x20 - LengthSize;

        [ByteSize(31)]
        [IgnoreDataMember]
        private uint8[] _value;

        public static explicit operator uint8[](Str31 p) => p.Value ?? [];
        public static explicit operator char[](Str31 p) => p.ToString()?.ToCharArray() ?? [];
        public static explicit operator string(Str31 p) => p.ToString() ?? string.Empty;
        public static explicit operator Str31(uint8[] v) => new(v);
        public static explicit operator Str31(char[] v) => new(v);
        public static explicit operator Str31(string v) => new(v);

        public Str31(uint8[] value)
        {
